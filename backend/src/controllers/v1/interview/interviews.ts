...
export const addInterview = asyncHandler(async (req: Request, res: Response) => {
	...
	if (!startTime || !endTime || !participants)
		throw new BadRequestError(
			"Please enter start Time, end Time and participants email list properly."
		);
	...
	if (startTime < Date.now())
		throw new BadRequestError(
			"Start time can not be before current Time. "
		);
	if (endTime < startTime)
		throw new BadRequestError("Meeting duration can not be negative. end Time is before start Time.");
	...
	for (let p of emailParticipants) {
		...
		sendEmail({
			email: p.email,
			subject: "Software Developer Engineer Role Interview",
...
export const updateInterview = asyncHandler(async (req: Request, res: Response) => {
	...
	for (let p of updateInterview.participants) {
		...
		sendEmail({
			email: p.email,
			subject: "[RESCHEDULED] Software Developer Engineer Role Interview",
			message: `Interview Rescheduled Timing: ${moment(startTime).format("hh:mm A")} - ${moment(endTime).format(
				"hh:mm A"
			)} on ${moment(startTime).format("DD-MM-YYYY")}`
		});
		console.log("mail sent!");
